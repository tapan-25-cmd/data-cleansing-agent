...
export function specFor(g: AccuracyGroup): GroupSpec {
  ...
  if (g.group === "A") {
    ...
    return {
      ...
      ways: [
        { title: "Confirmed by the product's own words", value: n(g.confirmed), fraction: `of ${n(g.products)} kept`, primary: true, text: `The description states the size or piece count Excel has. This is the only witness independent of the entry itself. ${g.wrong ? `${n(g.wrong)} were kept although the description states a different size.` : "None was kept against its description."}` },
        { title: "Consistent with the older size field", value: n(g.consistent), fraction: `of ${n(g.products)} kept`, text: "The older field holds the same value, the whole pack, or the same value within label rounding. Usually it is the same entry copied across, so this shows consistency, not correctness." },
        { title: "Nothing to compare with", value: n(g.unverified), fraction: `of ${n(g.products)} kept`, text: "No usable older field and no size in the description. The value is kept as entered." },
      ],
      waysNote: "No percentage is given for this group: whether the kept values are correct is measured against the Merchandising Team's cleansing once it is in the Data Lake (measure B in the email).",
